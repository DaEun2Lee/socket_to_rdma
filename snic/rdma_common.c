...
void serialize_epoll_ctl_dt(char *msg_data, int *epfd, int *op, int *fd, struct epoll_event *event) {
	...
	// memcpy(msg_data + 12, event, sizeof(struct epoll_event));
	...
	switch (*flag) {
		...
		case 1: {	// int
			*arg = malloc(sizeof(int));
			if (*arg == NULL) {
				errorInfoMes("malloc failed for int arg");
                return;
			}
			memcpy(*arg, msg_data + 8, sizeof(int));
			debugInfoMes("Deserialized int arg: %d", *(int *)(*arg));
			break;
		}
		case 2:	{	//struct flock
			*arg = malloc(sizeof(struct flock));
			if (*arg == NULL) {
                errorInfoMes("malloc failed for struct flock arg");
                return;
            }
            memcpy(*arg, msg_data + 8, sizeof(struct flock));
            debugInfoMes("Deserialized struct flock arg for cmd: %d", *cmd);
            break;
		}
		default:
